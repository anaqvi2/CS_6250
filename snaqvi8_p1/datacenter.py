...
class DataCenter(Topo):
	...
	swlinkConfig = {'bw': 10, 'delay': '0ms', 'max_queue_size': max_queue_size}
        ...
        for i in range(1, (fi+1)):
            switch_name = "mls"+str(i)
            mls = self.addSwitch(switch_name)
            self.addLink(tls, mls, port1=i, port2=1, **swlinkConfig)

            for j in range(1, (fi+1)):
                switch_name2 = "s"+str(i)+"x"+str(j)
                a_rs = self.addSwitch(switch_name2)
                self.addLink(mls, a_rs, port1=(1+j), port2=1, **swlinkConfig)

                for k in range(1, n+1):
                    switch_name3 = "h"+str(i)+"x"+str(j)+"x"+str(k)
                    a_h = self.addHost(switch_name3, **hostConfig)
                    self.addLink(a_h, a_rs, port1=0, port2=(1+k), **hostlinkConfig)
